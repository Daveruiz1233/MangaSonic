...
class ManhuaPlusParser extends BaseParser {
  ManhuaPlusParser() : super(siteName: 'ManhuaPlus', baseUrl: 'https://manhuaplus.top');

  @override
  Future<List<Manga>> fetchMangaList(int page) async {
    final response = await getRequest(page == 1 ? '$baseUrl/all-manga/' : '$baseUrl/all-manga/$page/');
    ...
    for (var element in elements) {
      ...
      if (aTag != null && imgTag != null) {
        ...
        final coverUrl = imgTag.attributes['data-original'] ?? imgTag.attributes['data-src'] ?? imgTag.attributes['src'] ?? '';
        
        if (url.isNotEmpty && title.isNotEmpty) {
          list.add(Manga(title: title, url: url, coverUrl: coverUrl, sourceId: 'manhuaplus'));
        }
      }
    }
    return list;
  ...
  Future<List<Manga>> searchManga(String query, int page) async {
    ...
    final response = await getRequest('$baseUrl/search?keyword=$query&page=$page');
    final document = parser.parse(response.body);
    final elements = document.querySelectorAll('.item');
  ...
  Future<MangaDetails> fetchMangaDetails(Manga manga) async {
    ...
    for (var item in infoItems) {
      ...
      if (label.contains('Author') || label.contains('Tác giả')) {
        ...
        genres = valueElement?.querySelectorAll('a').map((e) => e.text.trim()).toList() ?? [];
      }
    }
